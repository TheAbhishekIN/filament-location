  ...
  Alpine.data("locationPicker", (options = {}) => ({
    ...
    initializeMap() {
      ...
      const mapOptions = {
        ...
        mapId: "filament-location-map", // Required for AdvancedMarkerElement
      ...
      try {
        ...
        // Add marker using AdvancedMarkerElement (new) or fallback to Marker (legacy)
        ...
        if (google.maps.marker && google.maps.marker.AdvancedMarkerElement) {
          ...
          // Handle drag events for AdvancedMarkerElement
          if (!this.isDisabled) {
          ...
          // Fallback to legacy Marker for older Google Maps versions
          ...
          // Handle drag events for legacy Marker
          if (!this.isDisabled) {
            this.marker.addListener("dragend", (event) => {
              this.state = {
                latitude: event.latLng.lat(),
    ...
    handleLocationError(error) {
      ...
      switch (error.code) {
        case error.PERMISSION_DENIED:
          // Check if it's specifically the HTTPS requirement
          if (error.message && error.message.includes("secure origins")) {
            this.error =
              "⚠️ Location access requires HTTPS connection. Please access this page via HTTPS or localhost.";
          } else {
            this.error =
              "🚫 Location access denied. Please allow location permission in your browser and try again.";
          }
          break;
        case error.POSITION_UNAVAILABLE:
          ...
          break;
        case error.TIMEOUT:
          this.error = "⏱️ Location request timed out. Please try again.";
          break;
        default:
          // Fallback for other errors, including the secure origins error
          if (error.message && error.message.includes("secure origins")) {
            this.error =
              "🔒 Location services require a secure connection (HTTPS). Please use HTTPS or localhost to access location features.";
          } else {
            this.error =
...
document.addEventListener("DOMContentLoaded", function () {
  // Configuration will be injected by the service provider
  if (
    typeof window.filamentData !== "undefined" &&
    window.filamentData["drclothes/filament-location"]
  ) {
    window.filamentLocationConfig =
      window.filamentData["drclothes/filament-location"];
  }
});
